...
object BoardGenerator {
    ...
    fun generateBoard(
        ...
        for (r in 0 until rows) {
            for (c in 0 until columns) {
                ...
                val chosenType = if (candidatePool.isNotEmpty()) {
                    ...
                    // Fallback should never happen with >= 3 types, but handle safely
        ...
        // Safety assertion: In the unlikely event of fallback match, regenerate
        if (MatchDetector.hasAnyMatches(board)) {
            return generateBoard(rows, columns, random, allowedTypes)
        }
